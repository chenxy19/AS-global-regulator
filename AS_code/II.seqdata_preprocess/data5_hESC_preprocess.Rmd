...
sce <- SingleCellExperiment(list(counts=as.matrix(combined.counts[!is.spike,])), 
    rowData=features[!is.spike,], colData=pheno)
# the new version of SingleCellExperiment separately store endogeneous genes and spike-ins (in alternative experiment)
#isSpike(sce, "ERCC") <- is.spike  # this is for old version
altExp(sce,"ERCC")<-SingleCellExperiment(list(counts=as.matrix(combined.counts[is.spike,])     ), rowData=features[is.spike,], colData=pheno)
# Or, there is a simple function to separate data matrix into a smaller endogenous matrix and alternative experiment: splitAltExps(). is.spike和is.mito在这里的处理是等价的。
...
multiplot(cols=2,
    plotColData(sce, x="sample", y="sum"),   # sum of counts, or library size
    plotColData(sce, x="sample", y="detected"),  # number of detected features
    plotColData(sce, x="sample", y="altexps_ERCC_percent"),
    plotColData(sce, x="sample", y="altexps_mito_percent")
)
```
...
isOutlier: nmads:  the minimum number of MADs away from median; type: whether outliers should be looked for at both tails ("both").

```{r, remOutliers1}

libsize.drop <- isOutlier(sce$sum, nmads=3, type="lower", log=TRUE, batch=sce$sample) 
feature.drop <- isOutlier(sce$detected, nmads=3, type="lower", log=TRUE, batch=sce$sample) 
mito.drop <- isOutlier(sce$altexps_mito_percent, nmads=3, type="higher", batch=sce$sample)
spike.drop <- isOutlier(sce$altexps_ERCC_percent, nmads=3, type="higher", batch=sce$sample)
...
```{r}
#suppressWarnings({
#    lost <- calcAverage(counts(sce)[,discard])  #outdated
#    kept <- calcAverage(counts(sce)[,!discard])
#})
lost<-apply(counts(sce)[,discard],1,mean)
kept<-apply(counts(sce)[,!discard],1,mean)
...
# Do the same for ERCC and mito genes
sce_ERCC <- altExp(sce,"ERCC")
lost<-apply(counts(sce_ERCC)[,discard],1,mean)
kept<-apply(counts(sce_ERCC)[,!discard],1,mean)
log.vals <- edgeR::cpm(cbind(lost, kept), log=TRUE, prior=1)
logfc <- log.vals[,1] - log.vals[,2]
head(sort(logfc, decreasing=TRUE), 20)
lost <- pmax(lost, min(lost[lost > 0]))
...
points(lost, kept, col="red", pch=16)

sce_mito <- altExp(sce,"mito")
lost<-apply(counts(sce_mito)[,discard],1,mean)
kept<-apply(counts(sce_mito)[,!discard],1,mean)
log.vals <- edgeR::cpm(cbind(lost, kept), log=TRUE, prior=1)
logfc <- log.vals[,1] - log.vals[,2]
head(sort(logfc, decreasing=TRUE), 20)
lost <- pmax(lost, min(lost[lost > 0]))
kept <- pmax(kept, min(kept[kept > 0]))
points(lost, kept, col="dodgerblue", pch=16)
```
...
quickCluster assign cells to clusters of similar expression, and normalization is applied within each cluster, to deal with a heterogeneous cell population.
ComputeSumFactor is a normalization method that divide gene count by the scaling factor computed for each cell, plus 1 and take log. The scaling factor is computed for each cell with regard to the cluster the cell lies in.
...
Finally, the size factors are applied to compute normalized log-expression values. endogeneous genes and spike-ins use their respective scaling factor.

(An alternative method of normalization is to normalize endogeneous expression level with ERCC reads. The main practical difference is that spike-in normalization preserves differences in total RNA content between cells, whereas computeSumFactors and other non-DE methods do not.
...
```{r varbias}
scater::plotExplanatoryVariables(sce, variables=c("phenotype", "batch")) + fontsize
```
