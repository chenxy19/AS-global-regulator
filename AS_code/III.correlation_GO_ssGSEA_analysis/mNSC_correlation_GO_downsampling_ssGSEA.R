...
mean_psi_file = "mouseSC/mouseSC_psi_bimodal.csv"
mean_psi <- read.csv(mean_psi_file,header = TRUE,stringsAsFactors = FALSE, sep=" ") 
expr_file <- read.csv("mouseSC/mouseSC_clus1_expr.csv",header = TRUE,stringsAsFactors = FALSE, sep=" ") 
...
posi_corr_list_data3 <- e[[1]]
nega_corr_list_data3 <- e[[2]]

e <- find_gene_corr_FDR(expr_psi,"gene_mean_psi",cor_method="pearson", tail_col=2)
posi_corr_list_data3_psi2 <- e[[1]]
nega_corr_list_data3_psi2 <- e[[2]]
...
write.csv(ssgsea_score, "mouseSC/mouseSC_ssGSEA.csv")
...
posi_gsea_data3_bi <- e[[1]]
nega_gsea_data3_bi <- e[[2]]

e <- find_gene_corr_FDR(gsea_psi_bi,"gene_mean_psi",cor_method="pearson",tail_col=2)
posi_gsea_data3_bi_psi2<- e[[1]]
nega_gsea_data3_bi_psi2 <- e[[2]]
...
########## 4. mouseSC2 #########
# 1.1 Prepare mean_psi (nrows = sample) and expr_file (sample * gene name)
mean_psi_file = "mouseSC2/mouseSC2_psi_bimodal.csv"
mean_psi <- read.csv(mean_psi_file,header = TRUE,stringsAsFactors = FALSE, sep=" ") 
expr_file <- read.csv("mouseSC2/mouseSC2_qNSC_TPM_hg.csv",header = TRUE,stringsAsFactors = FALSE, sep=" ") 
expr_file<-as.data.frame(t(expr_file))
...
expr_psi <- merge_by_rownames(expr_file, mean_psi)
e <- find_gene_corr_FDR(expr_psi,"glbl_mean_psi",cor_method="pearson", tail_col=2)
posi_corr_list_data4 <- e[[1]]
nega_corr_list_data4 <- e[[2]]

e <- find_gene_corr_FDR(expr_psi,"gene_mean_psi",cor_method="pearson", tail_col=2)
posi_corr_list_data4_psi2 <- e[[1]]
nega_corr_list_data4_psi2 <- e[[2]]
...
write.csv(ssgsea_score, "mouseSC2/mouseSC2_ssGSEA.csv")

ssgsea_score<-t(ssgsea_score)
gsea_psi_bi <- merge_by_rownames(ssgsea_score, mean_psi)

e <- find_gene_corr_FDR(gsea_psi_bi,"glbl_mean_psi",cor_method="pearson",tail_col=2)
posi_gsea_data4_bi <- e[[1]]
nega_gsea_data4_bi <- e[[2]]

e <- find_gene_corr_FDR(gsea_psi_bi,"gene_mean_psi",cor_method="pearson",tail_col=2)
posi_gsea_data4_bi_psi2<- e[[1]]
nega_gsea_data4_bi_psi2 <- e[[2]]
# None of the pathway is correlated to both
